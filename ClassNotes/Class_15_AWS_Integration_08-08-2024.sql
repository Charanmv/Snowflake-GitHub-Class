...
CREATE OR REPLACE TABLE OUR_FIRST_DB.PUBLIC.movie_titles (
  ...
  STORAGE_PROVIDER = S3
  ENABLED = TRUE 
  STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::024848483653:role/snow-vitect-10am-role'
  STORAGE_ALLOWED_LOCATIONS = ('s3://vitech-snow10am-batch/csv/')
   COMMENT = 'This is an s3 integration object' 

   DESC integration s3_int;

   CREATE OR REPLACE stage stage_aws_csv
    URL = 's3://vitech-snow10am-batch/csv/'
    STORAGE_INTEGRATION = s3_int
...
CREATE OR REPLACE TABLE OUR_FIRST_DB.PUBLIC.movie_titles (
  ...
  CREATE OR REPLACE file format OUR_FIRST_DB.PUBLIC.csv_fileformat
    type = csv
    field_delimiter = ','
    skip_header = 1
    null_if = ('NULL','null')
    ...
    copy into OUR_FIRST_DB.PUBLIC.movie_titles  
  from @stage_aws_csv 
file_format = (FORMAT_NAME  = OUR_FIRST_DB.PUBLIC.csv_fileformat)
files =('netflix_titles.csv')
