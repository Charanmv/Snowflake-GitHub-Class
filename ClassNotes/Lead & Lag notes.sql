...
union
select distinct(first_name) from dependents;
...
1) LEAD AND LAG ???
2) nUMBER FUNCTIONS AT LEAST 10
...
--38 + 30 = 68   --> DATA TYPE SHOULD GET MATCH 
...
SELECT EMPLOYEE_ID,
      FIRST_NAME,
      SALARY ,
      lead(salary) OVER(ORDER BY SALARY DESC )  AS lead1,
      lag(salary) OVER(ORDER BY SALARY DESC )  AS lag1
      FROM EMPLOYEES 
)   SELECT * FROM CTE_EMP 
